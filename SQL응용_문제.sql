...
SELECT PROF_NAME AS 교수명, TRUNC(TO_DATE('2020-12-31', 'YYYY-MM-DD') - PROF_HIRED) AS 근속일
  FROM PROFESSOR_T;
...
SELECT STUD_NO AS 학번, STUD_NAME AS 학생명, STUD_HEIGHT AS 키
  FROM STUDENT_T
 WHERE STUD_HEIGHT = (SELECT MAX(STUD_HEIGHT) FROM STUDENT_T)
   OR STUD_HEIGHT = (SELECT MIN(STUD_HEIGHT) FROM STUDENT_T);  
...
SELECT DEPT_NO AS 학과번호, COUNT(*) AS 학생수
  FROM STUDENT_T
 GROUP BY DEPT_NO;
...
SELECT D.DEPT_NO AS 학과번호, D.DEPT_NAME AS 학과명, COUNT(S.STUD_NO) AS 학생수
  FROM DEPARTMENT_T D LEFT JOIN STUDENT_T S
