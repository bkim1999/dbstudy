...
CREATE SEQUENCE BOOK_SEQ START WITH 1 INCREMENT BY 1 ORDER;
...
CREATE SEQUENCE CUST_SEQ START WITH 1000 INCREMENT BY 1 ORDER;
...
CREATE SEQUENCE ORDER_SEQ START WITH 1 INCREMENT BY 1 ORDER;
...
UPDATE ORDER_T SET ORDER_ID = TO_CHAR(ORDER_DT, 'YYMMDD') || '-' || ORDER_ID;
...
SELECT CUST_NAME FROM CUSTOMER_T WHERE CUST_ID NOT IN (SELECT CUST_ID FROM ORDER_T);
...
SELECT O.ORDER_ID AS 주문번호, C.CUST_NAME AS 구매자, B.BOOK_NAME AS 책이름, B.PRICE * O.AMOUNT AS 총구매액, O.ORDER_DT 주문일자
  FROM BOOK_T B INNER JOIN ORDER_T O
...
SELECT (SELECT CUST_NAME FROM CUSTOMER_T WHERE O.CUST_ID = CUST_ID) AS 고객명,
       (SELECT BOOK_NAME FROM BOOK_T WHERE O.BOOK_ID = BOOK_ID) AS 책이름,
       ORDER_DT AS 주문일자
  FROM ORDER_T O
...
WITH NONORDERED AS (
    SELECT BOOK_ID, BOOK_NAME
      FROM BOOK_T
     WHERE BOOK_ID NOT IN (SELECT BOOK_ID FROM ORDER_T)
)
      
SELECT O.ORDER_ID, N.BOOK_ID, N.BOOK_NAME
  FROM NONORDERED N LEFT OUTER JOIN ORDER_T O
    ON N.BOOK_ID = O.BOOK_ID;
...
WITH MOST_EXPENSIVE AS (
    SELECT BOOK_ID, BOOK_NAME, PRICE
      FROM BOOK_T
     WHERE PRICE IN (SELECT MAX(PRICE) FROM BOOK_T)
)
      
SELECT NVL(C.CUST_NAME, 'NULL'), M.BOOK_NAME, M.PRICE
  FROM MOST_EXPENSIVE M LEFT OUTER JOIN ORDER_T O
    ON M.BOOK_ID = O.BOOK_ID LEFT OUTER JOIN CUSTOMER_T C
    ON O.CUST_ID = C.CUST_ID;

-- 2) 조인만

SELECT NVL(C.CUST_NAME, 'NULL'), B.BOOK_NAME, B.PRICE
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O
...
SELECT COUNT(*)
  FROM ORDER_T
 WHERE CUST_ID = (SELECT CUST_ID FROM CUSTOMER_T WHERE CUST_NAME = '김연아');
...
SELECT C.CUST_NAME, COUNT(DISTINCT B.BOOK_ID)
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
...
SELECT B.PUBLISHER, COUNT(O.ORDER_ID)
  FROM BOOK_T B LEFT JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID
...
SELECT C.CUST_NAME,  
       NVL(SUM(B.PRICE * O.AMOUNT), 0), 
       COUNT(O.ORDER_ID)
  FROM CUSTOMER_T C LEFT JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID LEFT JOIN BOOK_T B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUST_ID, C.CUST_NAME
 ORDER BY C.CUST_ID;
...
SELECT 고객명, 총구매액 
  FROM (SELECT C.CUST_NAME AS 고객명, 
               NVL(SUM(B.PRICE * O.AMOUNT), 0) AS 총구매액,
               RANK() OVER(ORDER BY NVL(SUM(B.PRICE * O.AMOUNT), 0) DESC) AS 순위
          FROM CUSTOMER_T C LEFT JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID LEFT JOIN BOOK_T B
            ON O.BOOK_ID = B.BOOK_ID
         GROUP BY C.CUST_ID, C.CUST_NAME)
